    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-6 bg-slate-50 dark:bg-[#0f1117]">
            ...
            <div className="w-full max-w-[440px] bg-white dark:bg-[#1e2130] border border-slate-200 dark:border-white/10 rounded-[24px] p-10 md:p-12 shadow-xl shadow-slate-200/50 dark:shadow-none relative z-10">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-8">Start tracking your budget today — free forever</p>

                {error && <div className="bg-red-50 text-red-600 border border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/30 rounded-xl p-3.5 text-sm mb-6">⚠️ {error}</div>}
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        <label className="block text-[13px] font-medium text-slate-600 dark:text-slate-400 mb-2">
                            ...
                            {form.password.length > 0 && (
                                <span className={`ml-2 text-[11px] font-semibold ${passwordStrength ? 'text-emerald-500' : 'text-amber-500'}`}>
                                    {passwordStrength ? '✓ Strong enough' : 'Min. 6 characters'}
                                </span>
                            )}
                        </label>
                    ...
                    <div>
                        <label className="block text-[13px] font-medium text-slate-600 dark:text-slate-400 mb-2">Confirm Password</label>
                        <div className="relative">
                            <input
                                className={`form-input pr-12 ${form.confirmPassword.length > 0 ? (passwordsMatch ? 'border-emerald-400 focus:border-emerald-500 focus:ring-emerald-500/20' : 'border-red-400 focus:border-red-400 focus:ring-red-400/20') : ''}`}
                    ...
                    <button
                        ...
                        disabled={loading || (form.confirmPassword.length > 0 && !passwordsMatch)}
                    >
                        {loading ? (
                ...
                <div className="text-center mt-6 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <Link to="/login" className="text-blue-600 dark:text-indigo-400 font-semibold hover:underline">Sign in</Link>
                </div>
